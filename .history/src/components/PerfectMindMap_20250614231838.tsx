...
const PerfectMindMap: React.FC<PerfectMindMapProps> = ({
  messages,
  uploadedFiles,
  sessionId,
  originalQuery = "",
  ...
  const [selectedNode, setSelectedNode] = useState<PerfectMindMapNode | null>(null);
  const [showDetails, setShowDetails] = useState(false);
  const [isSharing, setIsSharing] = useState(false);
  const [mindMapData, setMindMapData] = useState<PerfectMindMapData | null>(null);
  ...
  const mindMapCache = useRef<{ hash: string; data: PerfectMindMapData } | null>(null);
  ...
  const calculateDataHash = useCallback((messages: ChatMessage[], files: any[]) => {
  ...
  const generateMindMap = useCallback(async () => {
    ...
    try {
      ...
      const perfectedMindMapData = await perfectMindMapService.generatePerfectMindMap(
        messages,
        sessionId,
        uploadedFiles,
        originalQuery
      ...
      setEdges(perfectedMindMapData.edges);
  ...
  }, [messages, uploadedFiles, sessionId, originalQuery, onMindMapGenerated, calculateDataHash, fitView]);
  ...
  const filteredNodes = useMemo(() => {
    return nodes.filter(node => {
      ...
      switch (level) {
        case 1: return layerVisibility.layer1;
        case 2: return layerVisibility.layer2;
        case 3: return layerVisibility.layer3;
        case 4: return layerVisibility.layer4;
        default: return true;
      }
    });
  ...
  const downloadAsPNG = useCallback(async () => {
    ...
    try {
      ...
      if (mindMapElement) {
        ...
        link.download = `perfect-mind-map-${new Date().toISOString().split('T')[0]}.png`;
  ...
  const getNodeColor = useCallback((node: any) => {
    switch (node.data.level) {
      case 1: return '#8B5CF6'; // Purple - Central
      case 2: return '#3B82F6'; // Blue - Branches
      case 3: return '#10B981'; // Green - Files/Queries
      case 4: return '#EC4899'; // Pink - Analysis
          ...
          <Card className="bg-white/90 backdrop-blur-sm border border-gray-200 shadow-lg">
            ...
            <CardContent className="space-y-3 pt-0">
              ...
              <div className="space-y-2">
                <div className="text-xs font-medium text-gray-600">Layer Visibility</div>
                
                {[
                  { key: 'layer1', label: 'Central Brain', color: 'text-purple-600' },
                  { key: 'layer2', label: 'Categories', color: 'text-blue-600' },
                  ...
                  <div key={key} className="flex items-center justify-between">
                    ...
                    <Button
                      onClick={() => setLayerVisibility(prev => ({ ...prev, [key]: !prev[key as keyof typeof prev] }))}
                      size="sm"
                      variant="ghost"
              ...
              {mindMapData && (
                <>
                  ...
                  <div className="space-y-1">
                    <div className="text-xs font-medium text-gray-600">Statistics</div>
            ...
            <Card className="bg-white/95 backdrop-blur-sm border border-gray-200 shadow-xl w-80 max-h-96 overflow-y-auto">
              ...
              <CardContent className="space-y-3 pt-0">
                ...
                <Badge 
                  variant="secondary" 
                  className={`
                    ${selectedNode.data.level === 1 ? 'bg-purple-100 text-purple-700' : ''}
                    ${selectedNode.data.level === 2 ? 'bg-blue-100 text-blue-700' : ''}
                    ${selectedNode.data.level === 3 ? 'bg-green-100 text-green-700' : ''}
                    ${selectedNode.data.level === 4 ? 'bg-pink-100 text-pink-700' : ''}
                  `}
                >
                ...
                {selectedNode.data.fullText && (
                  <div>
                    <div className="text-xs font-medium text-gray-600 mb-1">Full Question</div>
                ...
                {selectedNode.data.content && (
                  <div>
                    <div className="text-xs font-medium text-gray-600 mb-1">Analysis Content</div>
                    <p className="text-xs text-gray-700 leading-relaxed bg-gray-50 p-2 rounded">
                      {selectedNode.data.content}
                    </p>
                  </div>
                ...
                {/* Keywords */}
                {selectedNode.data.keywords && selectedNode.data.keywords.length > 0 && (
                  <div>
                ...
                {selectedNode.data.timestamp && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-700">{selectedNode.data.timestamp}</span>
                  </div>
                )}
              </CardContent>
            </Card>
