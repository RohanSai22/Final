...
export const CentralBrainNode: React.FC<NodeProps> = ({ data, selected }) => {
  ...
  const isCentralData = data && 'totalFiles' in data && 'totalQueries' in data && 'sessionTitle' in data;
...
export const FileNode: React.FC<NodeProps> = ({ data, selected }) => {
  ...
  const fileName = data && 'fileName' in data ? data.fileName : data?.label || 'File';
