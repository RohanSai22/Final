...
export const CentralBrainNode: React.FC<NodeProps> = ({ data, selected }) => {
  const [isExpanding, setIsExpanding] = useState(false);

  // Type guard to check if this is central node data
  const isCentralData =
    data &&
    "totalFiles" in data &&
    "totalQueries" in data &&
    "sessionTitle" in data;
  const totalFiles = isCentralData ? data.totalFiles : 0;
  const totalQueries = isCentralData ? data.totalQueries : 0;
  const sessionTitle = isCentralData ? data.sessionTitle : "Session";
    ...
    <div className="central-brain-node relative">
      ...
      {selected && (
        <div className="absolute -top-16 left-1/2 transform -translate-x-1/2 bg-black/80 text-white px-3 py-2 rounded-lg text-xs z-10 whitespace-nowrap">
          📁 {totalFiles} files • ❓ {totalQueries} queries
        </div>
      )}
...
export const MainBranchNode: React.FC<NodeProps> = ({ data, selected }) => {
  const isFiles = data && "category" in data && data.category === "FILES";
  const icon = isFiles ? (
    <Folder className="w-6 h-6" />
  ) : (
    <HelpCircle className="w-6 h-6" />
  );
  const bgColor = isFiles
    ? "from-blue-500 to-cyan-500"
    : "from-green-500 to-emerald-500";
  const borderColor = isFiles ? "border-blue-300" : "border-green-300";
  const count = data && "count" in data ? data.count : 0;
  const summary = data && "summary" in data ? data.summary : "";

  return (
    <div className="main-branch-node relative">
      <div
        className={`bg-gradient-to-br ${bgColor} rounded-xl shadow-xl border-2 ${borderColor} p-4 min-w-[220px] hover:shadow-2xl transition-all duration-300 hover:scale-[1.02]`}
      >
        <div className="text-center text-white">
          <div className="flex items-center justify-center mb-2">{icon}</div>
          <h3 className="font-bold text-lg mb-2">{data?.label || "Branch"}</h3>
      ...
      <Handle
        type="target"
        position={Position.Top}
        className="w-3 h-3 bg-white border-2 border-current rounded-full"
        style={{ top: -6 }}
      />
      <Handle
        type="source"
...
export const FileNode: React.FC<NodeProps> = ({ data, selected }) => {
  ...
  const fileName =
    data && "fileName" in data ? data.fileName : data?.label || "File";
  const fileType = data && "fileType" in data ? data.fileType : "unknown";
  const fileSize = data && "fileSize" in data ? data.fileSize : 0;
  const uploadDate = data && "uploadDate" in data ? data.uploadDate : "";
  const summary = data && "summary" in data ? data.summary : "";
  const hasChildren = data && "hasChildren" in data ? data.hasChildren : false;
      ...
      <div className="bg-gradient-to-br from-indigo-50 to-blue-50 rounded-lg shadow-lg border-2 border-indigo-200 p-3 max-w-[240px] hover:shadow-xl transition-all duration-300 hover:border-indigo-300">
        <div className="text-center">
          ...
          <div className="flex items-center justify-between mb-2">
            <div className="text-lg">{getFileIcon(fileType)}</div>
...
export const QueryNode: React.FC<NodeProps<EnhancedMindMapNode["data"]>> = ({
  data,
  selected,
}) => {
    ...
    >
      <div className="bg-gradient-to-br from-emerald-50 to-green-50 rounded-lg shadow-lg border-2 border-emerald-200 p-3 max-w-[240px] hover:shadow-xl transition-all duration-300 hover:border-emerald-300">
        <div className="text-center">
          ...
          {/* Timestamp */}
          {"timestamp" in data && (
            <div className="text-xs text-gray-500 mb-2">{data.timestamp}</div>
          )}
      ...
      <Handle
        type="target"
        position={Position.Top}
        className="w-2 h-2 bg-emerald-400 border-2 border-white rounded-full"
        style={{ top: -4 }}
      />
      <Handle
        type="source"
...
export const FileAnalysisNode: React.FC<
  NodeProps<EnhancedMindMapNode["data"]>
> = ({ data, selected }) => {
  const getAnalysisIcon = (type: string) => {
    ...
    return (
      icons[type as keyof typeof icons] || <BarChart3 className="w-4 h-4" />
    );
  };
          ...
          <div
            className={`w-full bg-gradient-to-r ${
              "analysisType" in data
                ? getAnalysisColor(data.analysisType)
...
export const QueryAnalysisNode: React.FC<
  NodeProps<EnhancedMindMapNode["data"]>
> = ({ data, selected }) => {
  const getAnalysisIcon = (type: string) => {
    const icons = {
