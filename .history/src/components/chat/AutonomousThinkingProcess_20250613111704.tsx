...
const AutonomousThinkingProcess: React.FC<ThinkingProcessProps> = ({
  ...
  const renderThinkingBlock = (item: ThinkingStreamData, index: number) => {
    const baseClasses = "rounded-3xl p-6 mb-4 animate-in slide-in-from-left-4 duration-500";
    
    switch (item.type) {
      case "status":
        return (
          <Card key={index} className={`${baseClasses} bg-blue-50 border-blue-200`}>
            <CardContent className="p-0">
              <div className="flex items-center gap-3">
                <Brain className="w-5 h-5 text-blue-600" />
                <span className="text-blue-800 font-medium">{item.data.message}</span>
      ...
      case "sufficiency_check":
        return (
          <Card key={index} className={`${baseClasses} bg-amber-50 border-amber-200`}>
            <CardContent className="p-0">
              <div className="flex items-start gap-3">
                <AlertCircle className="w-5 h-5 text-amber-600 mt-0.5" />
                <div>
                  <h4 className="font-semibold text-amber-800 mb-2">Sufficiency Check</h4>
      ...
      case "plan":
        return (
          <Card key={index} className={`${baseClasses} bg-indigo-50 border-indigo-200`}>
            <CardContent className="p-0">
              <div className="flex items-start gap-3">
                <FileText className="w-5 h-5 text-indigo-600 mt-0.5" />
                <div className="flex-1">
                  <h4 className="font-semibold text-indigo-800 mb-3">Research Strategy</h4>
      ...
      case "searching_start":
        return (
          <Card key={index} className={`${baseClasses} bg-emerald-50 border-emerald-200`}>
            <CardContent className="p-0">
              <div className="flex items-center gap-3">
                <Search className="w-5 h-5 text-emerald-600" />
                <div>
                  <h4 className="font-semibold text-emerald-800">Searching</h4>
                  <p className="text-emerald-700 text-sm mt-1">{item.data.query}</p>
                </div>
                {isActive && <Loader2 className="w-4 h-4 animate-spin text-emerald-600" />}
              </div>
            </CardContent>
          </Card>
        );
      ...
      case "source_found":
        ...
        return (
          <Card key={index} className={`${baseClasses} bg-gray-50 border-gray-200`}>
            <CardContent className="p-0">
              <div className="flex items-center gap-3">
                {getSourceIcon(source.sourceType)}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <Badge className={`${getSourceColor(source.sourceType)} text-xs`}>
                      {source.sourceType}
                    </Badge>
                  </div>
      ...
      case "learning":
        return (
          <Card key={index} className={`${baseClasses} bg-violet-50 border-violet-200`}>
            <CardContent className="p-0">
              <div className="flex items-start gap-3">
                <BookOpen className="w-5 h-5 text-violet-600 mt-0.5" />
                <div>
                  <h4 className="font-semibold text-violet-800 mb-2">Learning</h4>
      ...
      case "reflection":
        return (
          <Card key={index} className={`${baseClasses} bg-rose-50 border-rose-200`}>
            <CardContent className="p-0">
              <div className="flex items-start gap-3">
                <CheckCircle className="w-5 h-5 text-rose-600 mt-0.5" />
                <div>
                  <h4 className="font-semibold text-rose-800 mb-2">Autonomous Reflection</h4>
                  <p className="text-rose-700">
                    <strong>Decision:</strong> {item.data.decision}
                  </p>
                  <p className="text-rose-700 mt-1 text-sm">{item.data.reason}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-3xl p-6 bg-blue-50 border-blue-200">
            <CardContent className="p-0">
              <div className="flex items-center gap-3">
                ...
                <span className="text-blue-800">Initializing autonomous research agent...</span>
              </div>
            </CardContent>
          </Card>
        )}
