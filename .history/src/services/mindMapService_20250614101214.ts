...
class MindMapService {
  ...
  private initializeAI() {
    try {
      ...
      if (this.apiKey) {
        ...
        console.log(
          "Mind Map Service: Google AI provider initialized successfully."
        );
      } else {
        console.error(
          "Mind Map Service: Google AI API key not found. VITE_GOOGLE_AI_API_KEY is not set."
        );
  ...
  public async generateMindMap(
    ...
    console.log(
      "MindMapService: Starting generateMindMap pipeline with new AtomicGraph approach."
    );
    try {
      if (!this.apiKey || !this.googleProvider) {
        console.error(
          "MindMapService: API key or provider not initialized. Returning fallback mind map."
        );
      ...
      if (chunks.length === 0) {
        console.warn(
          "MindMapService: No chunks created from content. Returning fallback."
        );
        return this.createFallbackMindMap(userQuery);
      }
      console.log(
        `MindMapService: Processing ${chunks.length} chunks for atomic knowledge extraction.`
      );

      let masterGraph: AtomicGraph = { entities: [], relationships: [] };
      let entityIdCounter = 0;

      const firstAtomicGraph = await this._extractAtomicKnowledge(
        chunks[0],
        userQuery
      );
      if (firstAtomicGraph.entities.length > 0) {
        ...
        const nameToIdMap = new Map(
          masterGraph.entities.map((e) => [e.name, e.id])
        );
        masterGraph.relationships = firstAtomicGraph.relationships
          .map((rel) => {
            ...
            console.warn(
              `MindMapService: Dropping relationship from first chunk due to missing entity mapping: ${rel.sourceName} -> ${rel.targetName}`
            );
            return null;
          })
          .filter(Boolean) as AtomicRelationship[];
        console.log(
          `MindMapService: Initial masterGraph created with ${masterGraph.entities.length} entities and ${masterGraph.relationships.length} relationships.`
        );
      }

      for (let i = 1; i < chunks.length; i++) {
        console.log(
          `MindMapService: Extracting atomic knowledge from chunk ${i + 1}/${
            chunks.length
          }...`
        );
        const atomicGraphFromChunk = await this._extractAtomicKnowledge(
          chunks[i],
          userQuery
        );
        if (
          atomicGraphFromChunk.entities.length > 0 ||
          atomicGraphFromChunk.relationships.length > 0
        ) {
          console.log(
            `MindMapService: Merging atomic graph from chunk ${
              i + 1
            } (Entities: ${atomicGraphFromChunk.entities.length}, Relations: ${
              atomicGraphFromChunk.relationships.length
            }) into masterGraph...`
          );
          masterGraph = await this._mergeAtomicGraphs(
            masterGraph,
            atomicGraphFromChunk,
            userQuery,
            entityIdCounter
          );
      ...
      if (masterGraph.entities.length === 0) {
        console.warn(
          "MindMapService: Master graph is empty after processing all chunks. Returning fallback."
        );
        return this.createFallbackMindMap(userQuery);
      }

      console.log(
        `MindMapService: Final MasterGraph has ${masterGraph.entities.length} entities and ${masterGraph.relationships.length} relationships.`
      );

      const rootEntityId = await this._findCentralTopicEntityId(
        masterGraph,
        userQuery
      );
      console.log(
        `MindMapService: Central topic entity ID determined as: ${rootEntityId}`
      );

      const hierarchicalRootNode = this._buildHierarchicalTreeFromMasterGraph(
        masterGraph,
        rootEntityId,
        userQuery
      );
      ...
      if (
        !hierarchicalRootNode ||
        hierarchicalRootNode.label === "Empty Graph" ||
        hierarchicalRootNode.label === "Error: No Entities"
      ) {
        console.error(
          "MindMapService: Failed to build hierarchical tree or tree is empty. Returning fallback."
        );
        return this.createFallbackMindMap(userQuery);
      }

      const mindMapData = this.convertToReactFlowFormat(
        hierarchicalRootNode,
        maxLevels
      );
      console.log(
        `MindMapService: Converted to ReactFlow format with ${mindMapData.nodes.length} nodes and ${mindMapData.edges.length} edges.`
      );
      return this.applyDagreLayout(mindMapData);
    } catch (error) {
      console.error(
        "MindMapService: Error in generateMindMap pipeline:",
        error
      );
      return this.createFallbackMindMap(userQuery);
    }
  ...
  private async _findCentralTopicEntityId(
    masterGraph: AtomicGraph,
    originalQuery: string
  ): Promise<string | null> {
    if (!masterGraph.entities || masterGraph.entities.length === 0) {
      console.warn(
        "MindMapService: _findCentralTopicEntityId - Master graph has no entities."
      );
      return null;
    }
    if (!this.googleProvider) {
      console.error(
        "MindMapService: _findCentralTopicEntityId - AI provider not initialized."
      );
    ...
    try {
      ...
      console.log(
        "MindMapService: _findCentralTopicEntityId - AI suggested central entity name:",
        centralEntityName
      );
      const centralEntity = masterGraph.entities.find(
        (e) => e.name === centralEntityName
      );
      if (centralEntity) {
        return centralEntity.id;
      } else {
        console.warn(
          "MindMapService: _findCentralTopicEntityId - AI suggested name not found in entities or response was not an entity name. Falling back. AI response:",
          centralEntityName
        );
        // Fallback: Check for partial match or first entity
        const partialMatch = masterGraph.entities.find(
          (e) =>
            e.name.includes(centralEntityName) ||
            centralEntityName.includes(e.name)
        );
      ...
      console.error(
        "MindMapService: _findCentralTopicEntityId - Error during AI call. Falling back to first entity.",
        error
      );
      return masterGraph.entities[0].id;
    }
  ...
  private _buildHierarchicalTreeFromMasterGraph(
    masterGraph: AtomicGraph,
    ...
    if (!rootEntityId || masterGraph.entities.length === 0) {
      console.warn(
        "MindMapService: _buildHierarchicalTreeFromMasterGraph - No rootEntityId or empty graph. Returning default root."
      );
      return {
        id: "root_fallback_" + uuidv4(),
        label: userQuery || "Graph Topic",
        relationship: "Central Query",
        level: 0,
        children: [],
        nodeType: "concept",
        summary: "This is the central query or topic.",
      };
    }
    ...
    if (!rootEntity) {
      console.error(
        "MindMapService: _buildHierarchicalTreeFromMasterGraph - Root entity ID not found in masterGraph. Returning default root."
      );
      return {
        id: "root_error_" + uuidv4(),
        label: userQuery || "Error: Root Not Found",
        relationship: "Error",
        level: 0,
        children: [],
        nodeType: "concept",
        summary: "The specified root entity was not found.",
      };
    }
    ...
    }> = [{ entityId: rootEntity.id, level: 0 }];
    ...
    while (head < queue.length) {
      ...
      if (visited.has(entityId) && parentId) {
        // Allow root to be "visited" once to initialize
      ...
      const currentNode: HierarchicalNode = {
        ...
        label: entity.name,
        relationship:
          relationshipLabel || (level === 0 ? "Central Topic" : "Related To"),
        level: level,
        children: [],
        summary: entity.description,
        nodeType: entity.type === "Concept" ? "concept" : entity.type,
      };
      ...
      if (parentId && hierarchicalNodes[parentId]) {
        ...
        if (
          !hierarchicalNodes[parentId].children.find(
            (c) => c.id === currentNode.id
          )
        ) {
      ...
      masterGraph.relationships.forEach((rel) => {
        if (rel.sourceId === entity.id && rel.targetId) {
          if (!visited.has(rel.targetId!)) {
            // Process child only if not fully processed as a parent
            queue.push({
            ...
            hierarchicalNodes[rel.targetId!] &&
            !hierarchicalNodes[entity.id].children.find(
              (c) => c.id === rel.targetId
            )
          ) {
    ...
    return (
      hierarchicalNodes[rootEntity.id] || {
        id: "root_final_fallback_" + uuidv4(),
        label: userQuery,
        relationship: "Central Query",
        level: 0,
        children: [],
        nodeType: "concept",
        summary: "Fallback root if hierarchy build failed.",
      }
    );
  }
  ...
  private _intelligentChunking(
    ...
    overlapPercentage: number = 0.15
  ): string[] {
    console.log("MindMapService: Starting _intelligentChunking.");
    const sentences: string[] = text.match(/[^.!?]+[.!?]+\s*|[^.!?]+$/g) || [];
    console.log(
      `MindMapService: _intelligentChunking - Split into ${sentences.length} sentences.`
    );
    ...
    for (let i = 0; i < sentences.length; i++) {
      ...
      if (
        ...
        lastChunkOverlapSentences.length > 0
      ) {
      ...
      if (
        currentChunkSentences.length === 0 ||
        currentCharCount + sentence.length <= chunkSize
      ) {
        ...
        if (currentChunkSentences.length > 0) {
          ...
          const numSentencesToOverlap = Math.max(
            1,
            Math.floor(currentChunkSentences.length * overlapPercentage)
          );
          lastChunkOverlapSentences = currentChunkSentences.slice(
            -numSentencesToOverlap
          );
    ...
    console.log(
      `MindMapService: _intelligentChunking - Total chunks created: ${chunks.length}`
    );
    return chunks;
  }
  ...
  async semanticChunking(content: string): Promise<string[]> {
    console.log(
      "MindMapService: Using _intelligentChunking for semantic chunking."
    );
    try {
      const chunks = this._intelligentChunking(content);
      return Promise.resolve(
        chunks.length > 0 ? chunks : this.fallbackChunking(content)
      );
    } catch (error) {
      console.error(
        "MindMapService: Error in _intelligentChunking, falling back to fallbackChunking:",
        error
      );
  ...
  private async generateAtomicTree(
    ...
    console.warn(
      "MindMapService: generateAtomicTree is deprecated. Using createSimpleTree as fallback. This method should ideally not be called in the new pipeline."
    );
    return this.createSimpleTree(
      chunk.substring(0, 50) + " (from deprecated generateAtomicTree)"
    );
  }
    ...
    const normalize = (node: any, level: number = 1): HierarchicalNode => {
      const label =
        node && typeof node.label === "string" && node.label.trim() !== ""
          ? node.label.trim()
          : fallbackLabel || "Untitled Concept";
      return {
        id: generateId(),
        label: label,
        relationship: (node && node.relationship) || "relates to",
        level,
  ...
  private async _extractAtomicKnowledge(
    chunk: string,
    originalQuery: string
  ): Promise<AtomicGraph> {
    if (!this.googleProvider) {
      console.error(
        "MindMapService: AI provider not initialized in _extractAtomicKnowledge."
      );
...
`;
  ...
  private async _mergeAtomicGraphs(
    masterGraph: AtomicGraph,
    newAtomicGraph: AtomicGraph,
    originalQuery: string,
    currentEntityIdCounter: number
  ): Promise<AtomicGraph> {
    let nextEntityIdCounter = currentEntityIdCounter;
    if (
      !newAtomicGraph ||
      (newAtomicGraph.entities.length === 0 &&
        newAtomicGraph.relationships.length === 0)
    ) {
      // Check relationships too
      console.log(
        "MindMapService: _mergeAtomicGraphs - New atomic graph is empty or has no content, returning masterGraph as is."
      );
      return masterGraph;
    }
    if (!masterGraph || masterGraph.entities.length === 0) {
      console.log(
        "MindMapService: _mergeAtomicGraphs - Master graph is empty, adopting newAtomicGraph."
      );
      ...
      const adoptedRelationships = newAtomicGraph.relationships
        .map((rel) => {
          ...
          console.warn(
            `MindMapService: _mergeAtomicGraphs (adopting) - Dropping relationship due to missing entity mapping: ${rel.sourceName} -> ${rel.targetName}`
          );
          return null;
        })
        .filter(Boolean) as AtomicRelationship[];
...
`;
  ...
  private createSimpleTree(content: string): HierarchicalNode {
    const generateId = () =>
      `${Date.now().toString(36)}-${Math.random().toString(36).substr(2, 9)}`;
    const label =
      content && content.trim() !== "" ? content.trim() : "Untitled Node";
    return {
      id: generateId(),
      label: label,
  ...
  private cloneNode(node: HierarchicalNode): HierarchicalNode {
    // Kept for potential utility, though less used in new pipeline
  ...
  private truncateTree(
    node: HierarchicalNode | AtomicEntity,
    maxDepth: number,
    currentDepth = 0,
    isAtomic = false
  ): any {
    ...
    if (currentDepth >= maxDepth) {
      if (isAtomic) {
        // node is AtomicEntity
        return { name: (node as AtomicEntity).name, id: node.id };
      } else {
        // node is HierarchicalNode
        return { label: (node as HierarchicalNode).label, id: node.id };
      }
    }
    if (isAtomic) {
      // AtomicEntity doesn't have children property directly
      return {
        name: (node as AtomicEntity).name,
        id: node.id,
        description: (node as AtomicEntity).description,
        type: (node as AtomicEntity).type,
      };
    }
    const hierarchicalNode = node as HierarchicalNode;
    return {
      label: hierarchicalNode.label,
      id: hierarchicalNode.id,
      children: (hierarchicalNode.children || []).map((child) =>
  ...
  private getNodeByPath(
    ...
    console.log(
      "Mind Map Service: getNodeByPath - Attempting to find node with path:",
      path,
      "in tree starting with label:",
      tree.label
    );
    ...
    try {
      ...
      console.error(
        "Mind Map Service: getNodeByPath - Error:",
        e,
        "Path:",
        path
      );
  ...
  private async findAnchorAndGraft(
    masterTree: HierarchicalNode,
    partialTree: HierarchicalNode
  ): Promise<HierarchicalNode> {
    console.warn(
      "MindMapService: findAnchorAndGraft is deprecated and part of the old hierarchical pipeline."
    );
  ...
  private async mergeTrees(
    ...
    console.warn(
      "MindMapService: mergeTrees is deprecated. New pipeline uses AtomicGraphs."
    );
  ...
  private convertToReactFlowFormat(
    ...
    const traverse = (node: HierarchicalNode, parentId?: string) => {
      ...
      const mindMapNode: MindMapNode = {
        ...
        }, // Ensure nodeType default
        style: this.getNodeStyle(node.level, node.nodeType),
      };
      if (
        !mindMapNode.id ||
        !mindMapNode.data ||
        typeof mindMapNode.data.label !== "string" ||
        mindMapNode.data.label.trim() === ""
      ) {
        return;
      }
      nodes.push(mindMapNode);
      if (parentId && parentId !== node.id) {
        const truncatedRelationship =
          (node.relationship || "relates to").length > MAX_RELATIONSHIP_LENGTH
            ? (node.relationship || "relates to").substring(
                0,
                MAX_RELATIONSHIP_LENGTH
              ) + "..."
            : node.relationship || "relates to";
        edges.push({
      ...
      (node.children || [])
        .filter(
          (child) =>
            ...
            typeof child.label !== "undefined"
        )
        .forEach((child) => traverse(child, node.id));
    };

    if (
      !tree ||
      typeof tree.id === "undefined" ||
      typeof tree.label === "undefined" ||
      tree.label.trim() === ""
    ) {
      return { nodes: [], edges: [] };
    }
    ...
    const nodes: MindMapNode[] = [
      {
        ...
        data: {
          label:
            userQuery.substring(0, 70) + (userQuery.length > 70 ? "..." : ""),
          level: 0,
          nodeType: "concept",
        },
      ...
      },
      {
        ...
        data: { label: "Key Concepts", level: 1, nodeType: "concept" },
        style: this.getNodeStyle(1),
      },
      {
        id: "concept2",
        position: { x: 0, y: 150 },
        data: { label: "Main Findings", level: 1, nodeType: "concept" },
        style: this.getNodeStyle(1),
      },
      {
        id: "concept3",
        position: { x: 250, y: 150 },
        data: { label: "Applications", level: 1, nodeType: "concept" },
        style: this.getNodeStyle(1),
      },
    ];
    const edges: MindMapEdge[] = [
    ...
    const nodes: MindMapNode[] = [
      {
        ...
        position: { x: 0, y: 0 },
        data: {
          label: topic.substring(0, 70) + (topic.length > 70 ? "..." : ""),
          level: 0,
          nodeType: "concept",
        },
        style: this.getNodeStyle(0),
