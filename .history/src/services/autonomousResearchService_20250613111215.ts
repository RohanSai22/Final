...
class AutonomousResearchService {
  ...
  private initializeAI() {
    try {
      ...
      console.log("Autonomous Research Agent initialized with Vercel AI SDK and Gemini 2.0 Flash Lite");
    } catch (error) {
  ...
  private async enforceRateLimit(): Promise<void> {
    ...
    if (this.requestCount >= this.rateLimitConfig.maxRequests) {
      const remainingTime = this.rateLimitConfig.windowMs - (now - this.windowStart);
  ...
  private async executeFileFirstAnalysis(
    ...
    try {
      ...
      const sufficiencyCheck = await this.checkFileSufficiency(userQuery, fileContent, callbacks);

      if (sufficiencyCheck.sufficient) {
        // Step A3: Generate file-only report
        await this.generateFileOnlyReport(userQuery, fileContent, researchMode, callbacks);
      } else {
  ...
  private async executeWebFirstResearch(
    ...
    try {
      ...
      // Strategic Planning
      const queries = await this.generateSearchQueries(userQuery, researchMode, fileContext, callbacks);
      ...
      for (const query of queries.slice(0, maxQueries)) {
        try {
          ...
          if (processedQueries >= 2) {
            ...
            if (sufficient && processedQueries >= (researchMode === "Normal" ? 2 : 3)) {
              break;
            }
          }
        } catch (searchError) {
...
Respond with only "SUFFICIENT" or "INSUFFICIENT" followed by a brief reason.`;
      ...
      const reason = response.replace(/SUFFICIENT|INSUFFICIENT/i, "").trim() || 
                    (sufficient ? "Files contain comprehensive information" : "Additional research needed");
  ...
  private async generateSearchQueries(
    ...
    try {
      ...
      if (researchMode === "Deep") {
        systemPrompt += " Focus on finding academic sources, recent developments, multiple perspectives, and detailed analysis.";
      } else {
        systemPrompt += " Focus on finding authoritative, up-to-date information from reliable sources.";
      }
  ...
  private async performWebSearch(
    ...
    try {
      ...
      if (fileContext) {
        systemPrompt += ` Context from user files: ${fileContext.substring(0, 2000)}`;
      }
...
Respond with "SUFFICIENT" if the information is adequate, or "INSUFFICIENT" with a brief reason.`;
      ...
      callbacks.onThinkingData({
        ...
        data: {
          ...
          reason: response.replace(/SUFFICIENT|INSUFFICIENT/i, "").trim() || 
...
Return only valid JSON.`;
  ...
  async expandMindMapNode(
    ...
    try {
      ...
      const prompt = `Expand the mind map node "${targetNode.label}" with 2-3 relevant child nodes.
...
{
  "newNodes": [
    {"id": "unique-id", "label": "Node Label", "level": ${targetNode.level + 1}, "data": {"label": "Node Label", "summary": "Brief description", "level": ${targetNode.level + 1}}}
  ],
  "newEdges": [
  ...
  private categorizeSource(url: string): "Academic" | "Government" | "News" | "Research" | "Web" {
    ...
    if (urlLower.includes(".edu") || urlLower.includes("scholar.google") || 
        urlLower.includes("jstor") || urlLower.includes("pubmed") || 
        urlLower.includes("arxiv") || urlLower.includes("researchgate")) {
      return "Academic";
    }

    if (urlLower.includes(".gov") || urlLower.includes(".mil") || 
  ...
  private validateAndFormatMindMap(data: any, userQuery: string): MindMapData {
    try {
      ...
      const nodes = data.nodes.map((node: any, index: number) => ({
        ...
        level: node.level || 1,
        position: node.position || { x: index * 200, y: (node.level || 1) * 100 },
        data: {
          label: node.label || "Node",
  ...
  private createFallbackMindMap(userQuery: string, sources: Source[]): MindMapData {
    ...
    categories.forEach((category, index) => {
      ...
      nodes.push({
        ...
        data: { label: `${category} Sources`, summary: `Sources from ${category.toLowerCase()} category`, level: 2 }
      });
