...
export interface ChatSession {
  ...
  advancedMindMapData?: AdvancedMindMapData | null;
  createdAt: Date;
  lastUpdated: Date;
  ...
  saveSession(session: ChatSession): void {
    try {
      ...
      const existingIndex = sessions.findIndex((s) => s.id === session.id);      const summary: ChatSessionSummary = {
        ...
        hasMindMap: !!session.fullMindMapData || !!session.advancedMindMapData,
        ...
        // Enhanced metadata
        topicCount: session.advancedMindMapData?.topics?.length || 0,
        relevanceScore: this.calculateSessionRelevanceScore(session),
        mindMapVersion: session.advancedMindMapData?.metadata?.version || "1.0",
        lastMindMapUpdate: session.advancedMindMapData?.metadata?.generatedAt 
          ? new Date(session.advancedMindMapData.metadata.generatedAt) 
          : undefined,
      };
