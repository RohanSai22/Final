...
class PerfectMindMapService {
  ...
  private mindMapCache = new Map<
    string,
    { hash: string; data: PerfectMindMapData }
  >();
  ...
  public async generatePerfectMindMap(
    ...
    try {
      ...
      console.log(
        `🎉 Generated perfect mind map with ${mindMapData.nodes.length} nodes and ${mindMapData.edges.length} edges`
      );
      return mindMapData;
    } catch (error) {
  ...
  private async generateCompleteLayeredStructure(
    ...
    const centralNode = this.createCentralBrainNode(
      processedData,
      originalQuery
    );
    ...
    const { filesBranch, questionsBranch } =
      this.createMainBranchNodes(processedData);
    nodes.push(filesBranch, questionsBranch);

    // Connect central to branches
    edges.push(
      this.createHierarchicalEdge(
        centralNode.id,
        filesBranch.id,
        "organizes",
        "#8B5CF6"
    ...
    const fileAnalysisNodes = await this.createFileAnalysisNodes(
      fileNodes,
    ...
    fileAnalysisNodes.forEach((analysisNode) => {
      if (analysisNode.data.parentId) {
        edges.push(
          this.createAnalysisEdge(
            analysisNode.data.parentId,
            analysisNode.id,
            "analyzes",
            "#6366F1"
          )
    ...
    const queryAnalysisNodes = await this.createQueryAnalysisNodes(
      queryNodes,
  ...
  private createCentralBrainNode(
    ...
    const userMessages = processedData.messages.filter(
      (m) => m.type === "user"
    );
  ...
  private createQueryNodes(messages: ChatMessage[]): PerfectMindMapNode[] {
    ...
    limitedMessages.forEach((message, index) => {
      const queryLabel = `💬 ${message.content.substring(0, 45)}${
      ...
      for (const analysis of analysisTypes) {
        const analysisNode: PerfectMindMapNode = {
          ...
          data: {
            ...
            content: await this.generateFileAnalysisContent(
              file,
              analysis.type
            ),
  ...
  private getNodeDimensions(node: PerfectMindMapNode): {
    width: number;
    height: number;
  } {
    switch (node.data.level) {
      case 1:
        return { width: 240, height: 140 }; // Central brain
      case 2:
        return { width: 200, height: 120 }; // Main branches
      case 3:
        return { width: 180, height: 100 }; // File/Query nodes
      case 4:
        return { width: 160, height: 80 }; // Analysis nodes
      default:
  ...
  private getAntiClusteringSpacing(
    ...
    if (node.data.level === 4) {
      ...
      const sameParentNodes = allNodes.filter(
        (n) => n.data.level === 4 && n.data.parentId === node.data.parentId
      );
  ...
  private async generateFileAnalysisContent(
    file: any,
    analysisType: string
  ): Promise<string> {
    if (!this.apiKey) {
...
Analysis type: ${analysisType}`;
  ...
  private getFileAnalysisFallback(file: any, analysisType: string): string {
    const fallbacks = {
      content: `Content analysis of ${
        file.name || "file"
      } reveals structured information with key data points and relationships.`,
      ...
      summary: `Comprehensive overview of ${
        file.name || "file"
      } covering main concepts, findings, and practical applications.`,
      topics: `Topics categorized: technical concepts, research methodologies, analytical results, and strategic recommendations.`,
    };
    return (
      fallbacks[analysisType as keyof typeof fallbacks] ||
      "Analysis content available for detailed review."
    );
  }

  private getQueryAnalysisFallback(
    userMessage: ChatMessage,
    analysisType: string
  ): string {
    const fallbacks = {
      thinking:
        "AI thinking process: comprehensive question analysis, context evaluation, and strategic response formulation.",
      reasoning:
        "Reasoning steps: logical progression from question understanding through evidence analysis to conclusion formation.",
      evidence:
        "Evidence sources: relevant data integration, supporting documentation review, and verification methodology.",
      conclusions:
        "Final conclusions: synthesized findings, actionable insights, and recommended next steps for implementation.",
      related:
        "Related concepts: connected knowledge areas, broader implications, and potential follow-up exploration topics.",
    };
    return (
      fallbacks[analysisType as keyof typeof fallbacks] ||
      "Analysis content available for detailed review."
