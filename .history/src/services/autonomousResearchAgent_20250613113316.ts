...
class AutonomousResearchAgent {
  ...
  private async executeFileFirstAnalysis(
    ...
    try {
      ...
      const answers = await this.parallelInterrogation(
        userQuery,
        chunks,
        callbacks
      );
      ...
      const sufficientAnswers = answers.filter(
        (answer) => answer && answer !== "INSUFFICIENT" && answer.length > 50
      );
      ...
      if (sufficientAnswers.length === 0) {
        callbacks.onThinkingData({
          ...
          data: {
            status: "insufficient",
            reason:
              "The provided documents do not contain sufficient information. Beginning web research.",
          },
          timestamp: Date.now(),
        });
...
Synthesized answer:`,
  ...
  private async generateSearchQueries(
    ...
    try {
      ...
      if (researchMode === "Deep") {
        prompt +=
          " Focus on academic sources, recent developments, multiple perspectives, and detailed analysis.";
      }

      if (fileContext) {
        prompt +=
          " The user provided documents that were insufficient. Generate queries that expand beyond the file content.";
      }
  ...
  private async performWebSearch(
    ...
    try {
      ...
      if (fileContext) {
        prompt += `\n\nContext from user files: ${fileContext.substring(
          0,
          500
        )}...`;
      }
  ...
  async expandMindMapNode(
    ...
    try {
      ...
      const prompt = `Expand the mind map node "${
        node.data.label
      }" with 2-3 child concepts.

Original query: ${userQuery}
Node to expand: ${node.data.label}
Current level: ${node.data.level}

Generate child nodes as JSON array with: id, label, summary, level (${
        node.data.level + 1
      })
