      ...
      for await (const delta of result.fullStream) {
        if (delta.type === "text-delta") {
          ...
          const source: Source = {
            ...
            url: delta.url || `https://search-${sources.length + 1}.com`,
            title: delta.title || `Research Source ${sources.length + 1}`,
          ...
          for (const sourceData of delta.sources) {
            const source: Source = {
              ...
              url: sourceData.url || `https://search-${sources.length + 1}.com`,
              title: sourceData.title || `Research Source ${sources.length + 1}`,
