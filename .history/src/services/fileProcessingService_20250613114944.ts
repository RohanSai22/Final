...
class FileProcessingService {
  ...
  getProcessingStats(results: FileProcessingResult[]): {
    ...
    return {
      ...
      totalWords: successfulResults.reduce((sum, r) => sum + r.metadata.wordCount, 0),
      totalCharacters: successfulResults.reduce((sum, r) => sum + r.metadata.charCount, 0),
      averageFileSize: results.length > 0 
        ? results.reduce((sum, r) => sum + r.metadata.fileSize, 0) / results.length 
        : 0
    };
  }
