...
class ChatSessionStorageService {
  ...
  saveSession(session: ChatSession): void {
    try {
      ...
      localStorage.setItem(`chatSession_${session.id}`, JSON.stringify(session));
      ...
      const summary: ChatSessionSummary = {
        id: session.id,
        title: session.originalQuery.length > 50 
          ? session.originalQuery.substring(0, 50) + '...' 
          : session.originalQuery,
        originalQuery: session.originalQuery,
        lastUpdated: new Date(session.lastUpdated),
        messageCount: session.messages.length,
        hasFiles: session.uploadedFileMetadata && session.uploadedFileMetadata.length > 0,
        hasMindMap: !!session.fullMindMapData,
        hasThinking: session.messages.some(msg => 
          msg.type === 'ai' && (msg.thinkingStreamData || msg.thinking)
        ),
  ...
  deleteSession(sessionId: string): void {
    try {
      ...
      keys.forEach(key => {
        if (key.startsWith(`thinking_process_${sessionId}_`) || 
  ...
  saveThinkingProcess(messageId: string, thinkingData: ThinkingStreamData[]): void {
    try {
      localStorage.setItem(`thinking_process_${messageId}`, JSON.stringify(thinkingData));
  ...
  loadThinkingProcess(messageId: string): ThinkingStreamData[] | null {
    try {
