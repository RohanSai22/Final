...
const ChatPage = () => {
  ...
  }, [messages, originalQuery, uploadedFiles, isAutonomousMode, fullMindMapData, isProcessing, currentSessionId]);
  ...
  useEffect(() => {
    ...
    if (location.state) { 
      ...
      console.log("ChatPage: New navigation state present, creating new session and initializing.");
        ...
        await autonomousResearchAgent.conductResearch(query, currentFiles, researchMode, {
            ...
            onFinalAnswer: async (report) => {
              ...
              if (thinkingForStorage.length > 0) {
                ...
                chatSessionStorage.saveThinkingProcess(aiMessageId, thinkingForStorage);
              }
    ...
    try {
      ...
      if (session) {
        ...
        if (session.fullMindMapData) {
          const initialNodes = session.fullMindMapData.nodes.filter(node => node.data.level <= INITIAL_DISPLAY_LEVEL);
          const initialNodeIds = new Set(initialNodes.map(node => node.id));
          const initialEdges = session.fullMindMapData.edges.filter(edge => initialNodeIds.has(edge.source) && initialNodeIds.has(edge.target));
          setDisplayedMindMapData({ nodes: initialNodes, edges: initialEdges });
        }
      }
    } catch (error) {
  ...
  const handleNewChat = () => {
    ...
    toast({ title: "New Chat Started", description: "Previous session saved and new session created." });
  };
  ...
  const generateAndShowMindMap = async () => {
    ...
    try {
      ...
      if (mindMap) {
        ...
        const initialNodes = mindMap.nodes.filter(node => node.data.level <= INITIAL_DISPLAY_LEVEL);
        const initialNodeIds = new Set(initialNodes.map(node => node.id));
        const initialEdges = mindMap.edges.filter(edge => 
  ...
  const handleNodeExpand = async (nodeId: string) => {
    ...
    try {
      ...
      const childEdges = fullMindMapData.edges.filter(edge => 
        nodeChildren.includes(edge.target) && edge.source === nodeId
      );

      // Add children to displayed mind map
      const existingNodeIds = new Set(displayedMindMapData.nodes.map(n => n.id));
      const newNodes = childNodes.filter(node => !existingNodeIds.has(node.id));
      
      const existingEdgeIds = new Set(displayedMindMapData.edges.map(e => e.id));
      const newEdges = childEdges.filter(edge => !existingEdgeIds.has(edge.id));
  ...
  const handleNodeReveal = async (nodeId: string) => {
    ...
    try {
      ...
      const revealedNodes = fullMindMapData.nodes.filter(node => 
        node.data.level <= newMaxLevel
      );
      
      const revealedNodeIds = new Set(revealedNodes.map(node => node.id));
      const revealedEdges = fullMindMapData.edges.filter(edge => 
