        ...
        await autonomousResearchAgent.conductResearch(
          ...
          researchMode,
          {
            ...
            onFinalAnswer: async (report) => {
              ...
              // Generate mind map after research is complete
              try {
                const mindMap = await autonomousResearchAgent.generateMindMap(
                  query,
                  report.content,
                  report.sources.map(url => ({ url, title: url }))
                );
                setMindMapData(mindMap);
              } catch (mindMapError) {
                console.error("Mind map generation error:", mindMapError);
                // Continue without mind map
              }
