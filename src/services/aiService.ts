...
class AIService {
  ...
  async processResearch(
    ...
    try {
      ...
      // Generate mind map if we have a final report
      if (finalReport) {
        callbacks.onProgress("Creating Mind Map", 85);
        ...
        try {
          ...
          console.warn(
            "Mind map generation failed, using fallback:",
            mindMapError
          );
          mindMapData = await mindMapService.generateSimpleMindMap(
            request.query,
            this.extractKeyConceptsFromReport(finalReport.content)
          );
        }
      }
